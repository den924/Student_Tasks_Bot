...
function registerAdminHandlers(bot) {
            ...
            "🛠 Панель администратора:",
            {
                reply_markup: {
                    inline_keyboard: [
                        [
                            {
                                text: "📋 Заявки на группы",
                                callback_data: "admin_group_requests"
                            }
                        ],
                        [
                            {
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (query.data !== "admin_group_requests") {
            return;
        }
        ...
        try {
            ...
            for (const request of requests) {
                ...
                const firstName =
                    request.user?.firstName || "Не указано";
                        ...
                        reply_markup: {
                            inline_keyboard: [
                                [
                                    {
                                        text: "✅ Одобрить",
                                        callback_data: `approve_group_${request.id}`
                                    },
                                    {
                                        text: "❌ Отклонить",
                                        callback_data: `reject_group_${request.id}`
                                    }
                                ]
                            ]
                        }
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("approve_group_")) {
            return;
        }
        ...
        const requestId = Number(
            query.data.replace("approve_group_", "")
        );

        try {

            const request = await GroupRequest.findByPk(
                requestId,
                {
                    include: [
                        {
            ...
            console.log(
                `✅ Заявка №${request.id} одобрена администратором ${telegramId}`
            );
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("reject_group_")) {
            return;
        }
        ...
        const requestId = Number(
            query.data.replace("reject_group_", "")
        );
        ...
        try {
            ...
            console.log(
                `❌ Заявка №${request.id} отклонена администратором ${telegramId}`
            );
    ...
    bot.on("callback_query", async (query) => {
        ...
        try {
            ...
            const keyboard = groups.map((group) => [
                {
                    text: `🏫 ${group.name}`,
                    callback_data: `admin_role_group_${group.id}`
                }
            ]);
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("admin_role_group_")) {
            return;
        }
        ...
        const groupId = Number(
            query.data.replace("admin_role_group_", "")
        );
        ...
        try {
            ...
            for (const member of members) {
                ...
                keyboard.push([
                    {
                        text: `${displayName} — ${member.role}`,
                        callback_data: `admin_role_user_${group.id}_${member.userId}`
                    }
                ]);
            }
            ...
            await bot.answerCallbackQuery(
                query.id,
                {
                    text: "Ошибка загрузки участников",
                    show_alert: true
                }
            );
        }
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("admin_role_user_")) {
            return;
        }
                ...
                `🎓 Текущая роль: ${member.role}`,
                {
                    reply_markup: {
                        inline_keyboard: [
                            [
                                {
                                    text: "🎓 Назначить куратором",
                                    callback_data: `set_curator_${groupId}_${userId}`
                                }
                            ],
                            [
                                {
                                    text: "👤 Назначить участником",
                                    callback_data: `set_member_${groupId}_${userId}`
                                }
                            ]
                        ]
                    }
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("set_curator_")) {
            return;
        }
        ...
        try {
            ...
            await bot.answerCallbackQuery(
                query.id,
                {
                    text: "Пользователь назначен куратором"
                }
            );
    ...
    bot.on("callback_query", async (query) => {
        ...
        if (!query.data.startsWith("set_member_")) {
            return;
        }
        ...
        try {
            ...
            await bot.answerCallbackQuery(
                query.id,
                {
                    text: "Пользователь назначен участником"
                }
            );
